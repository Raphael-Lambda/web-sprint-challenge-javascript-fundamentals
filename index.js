...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// This variable "internal" is part of the higher order function "myFunction" closure, so the function that are called inside this higher order function can access this variable. 
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(anArray){
    const displayNames = [];
    anArray.forEach(x => displayNames.push(`name: ${x.animal_name}, scientific: ${x.scientific_name}`))
    return displayNames
  }
  ...
  */
  ...
  function lowerCaseNames(anArray){
    return anArray.map(x => x.animal_name.toLowerCase())
      }
  ...
  */
  ...
  function lowPopulationAnimals(anArray){
    return anArray.filter(x => x.population < 5)
  }
  ...
  */
  ...
  function USApop(anArray){
    return anArray.reduce((total, x) => { return total + x.population}, 0)
  }
...
// // greetings
// (first, last) => { return (`Hello ${first} ${last}, nice to meet you!`)}
...
/* 🐴🐴🐴 Step 4: Create a new object that uses cuboid 🐴🐴🐴
...
// console.log(Cube2.surfaceArea()); // 84
